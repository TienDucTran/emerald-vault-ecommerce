...
export function AccountInfoCards() {
  const progressPercent = Math.min(100, (MOCK_TIER.current_spend / MOCK_TIER.next_tier_threshold) * 100);
  return (
    <div className="flex flex-col gap-6">
      <div className="rounded-lg border border-gold/20 bg-surface-emerald/30 p-6">
        <h3 className="mb-6 border-b border-gold/10 pb-2 font-heading text-sm text-gold">THÔNG TIN TÀI KHOẢN</h3>
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <span className="text-[13px] text-text-muted">Mã thành viên</span>
            <span className="font-mono text-sm text-gold-champagne">{MOCK_ACCOUNT.account_code}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-[13px] text-text-muted">Ngày gia nhập</span>
            <span className="text-sm text-text-base">{MOCK_ACCOUNT.created_at}</span>
          </div>
          <div className="flex items-center justify-between">
      ...
      <div className="relative overflow-hidden rounded-lg border border-gold/40 bg-surface p-6">
        <div className="pointer-events-none absolute inset-0 opacity-30" style={{ background: 'linear-gradient(90deg, rgba(242,202,80,0) 0%, rgba(242,202,80,0.2) 50%, rgba(242,202,80,0) 100%)' }} />
        <div className="relative flex flex-col gap-4">
          ...
          <div className="flex flex-col items-center gap-1 py-4">
            <span className="font-heading text-[40px] font-bold leading-none text-gold-champagne">TIER {MOCK_TIER.tier}</span>
            <span className="font-heading text-[10px] uppercase tracking-[0.2em] text-gold">{MOCK_TIER.tier_name}</span>
          </div>
          <div className="flex flex-col gap-2">
            <div className="relative h-1 w-full overflow-hidden rounded-full bg-surface-emerald">
              <div className="absolute inset-0 rounded-full bg-gradient-gold shadow-[0_0_8px_rgba(242,202,80,0.5)]" style={{ width: progressPercent + '%' }} />
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[11px] text-text-muted">{formatVNDShort(MOCK_TIER.current_spend)}đ</span>
              <span className="text-[11px] text-gold">{formatVNDShort(MOCK_TIER.next_tier_threshold)}đ để lên hạng</span>
            </div>
          </div>
        </div>
      </div>
