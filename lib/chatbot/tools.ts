...
export const getRelatedProducts = tool({
  ...
  execute: async ({ productId, category, material, excludeProductId, limit }) => {
    try {
      const supabase = createAdminClient();
      let q = supabase
        .from('products')
...
export const getUpcomingProducts = tool({
  ...
  execute: async ({ category, material, limit }) => {
    try {
      const supabase = createAdminClient();
      let q = supabase
...
export const getActivePromotions = tool({
  ...
  execute: async ({ category, minOrderValue }) => {
    try {
      const supabase = createAdminClient();
