...
**🆕 Checkout flow optimization — Idempotency + MoMo timeout** (sprint 2026-08-13): ✅ done. Audit checkout flow hiện tại, phát hiện 2 gap reliability: (1) double checkout khi user bấm submit 2 lần hoặc refresh — chỉ guard `submitting` state client-side, không có idempotency key server-side; (2) MoMo orders stuck NEW/RESERVED mãi nếu user đóng app sau redirect (cron 0030 chỉ cleanup BANK_TRANSFER > 24h, không có MoMo timeout). Fix:
- **P0 — Idempotency key cho `/api/orders`** (migration `0038_order_idempotency_keys.sql` + `checkout-form.tsx` + `app/api/orders/route.ts`): Bảng `order_idempotency_keys` (key + user_id + order_id + expires_at 24h, unique index `(key, user_id)`, RLS service_role only, cron cleanup daily 02:30). Client generate `crypto.randomUUID()` khi mount checkout, persist trong `sessionStorage['checkout_idempotency_key']`, gửi header `X-Idempotency-Key`. Server lookup key trước khi tạo order → nếu tồn tại + có `order_id` → return order cũ (`idempotent: true` flag). Save key sau khi order tạo thành công (non-fatal nếu fail). Clear sessionStorage key khi order thành công (cả 2 nhánh MoMo + COD/BANK). Fallback in-memory key nếu sessionStorage không khả dụng. Race condition: 2 request song song cùng key → unique constraint reject request thứ 2 (DB-level guarantee).
- **P1 — MoMo timeout auto-cancel** (migration `0039_momo_timeout_cron.sql`): Function `cancel_stale_momo_orders()` — tìm MoMo orders `NEW > 15 phút` chưa nhận IPN → cancel + release `inventory_locks` CONVERTED → RELEASED + restore `products` RESERVED → AVAILABLE + set `orders.status = CANCELLED`, `payment_status = FAILED` + log `order_timeline` event CANCELLED với reason `MOMO_TIMEOUT`. Cron mỗi 15 phút (lúc :02, :17, :32, :47 — offset khỏi release-expired-locks). Giải quyết: order không còn stuck PENDING mãi nếu user đóng app sau redirect MoMo.
- **Non-fatal design**: Nếu migration 0038 chưa apply → idempotency lookup/save skip (catch error, log, continue), order vẫn tạo bình thường. Không break existing flow.
- **Cần apply 2 migrations** qua Supabase Dashboard → SQL Editor: `0038_order_idempotency_keys.sql` + `0039_momo_timeout_cron.sql`.
